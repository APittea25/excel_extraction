...
# --- Graphviz graph with file-based clustering and terminal node coloring ---
def create_dependency_graph(dependencies, named_refs):
    dot = graphviz.Digraph()
    dot.attr(compound='true', rankdir='LR')

    file_clusters = defaultdict(list)
    all_nodes = set(named_refs.keys())
    nodes_with_outgoing = set()
    for sources in dependencies.values():
        nodes_with_outgoing.update(sources)
    terminal_nodes = all_nodes - nodes_with_outgoing

    for label, info in named_refs.items():
        file_clusters[info["file"].strip()].append(label)

    for i, (file_name, labels) in enumerate(file_clusters.items()):
        with dot.subgraph(name=f"cluster_{i}") as c:
            c.attr(label=file_name)
            c.attr(style="filled", color="lightgrey")
            for label in labels:
                if label in terminal_nodes:
                    c.node(label, style="filled", fillcolor="lightcoral")
                else:
                    c.node(label)
...
if uploaded_files:
    ...
    if combined_named_refs:
        ...
        dot = create_dependency_graph(dependencies, combined_named_refs)
