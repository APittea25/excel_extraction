...
# ↓↓↓ Removed print button and JS expander auto-open here

# Manual mapping for external references like [1], [2], etc.
...
if uploaded_files:
    ...
    # Collect named-references across all uploaded workbooks
    ...
    def remap_formula(formula, current_file, current_sheet):
        ...
        def remap_single_cell(ref, default_file, default_sheet):
            if "!" in ref:
                ...
                if match:
                    ext = match.group(0)
                    ext_fn = external_refs.get(ext, ext)
                    return f"[{ext_fn}]{ref}"
            ...
            for r in range(sr, er + 1):
                for c in range(sc, ec + 1):
                    ...
                    if key in all_named_cell_map:
                        ...
                        labels.add(f"[{default_file}]{sheet_name}!{cell_address(r,c)}")
        ...
        pattern = (
            ...
            r"(?::\$?[A-Z]{1,3}\$?[0-9]{1,7})?"
            r"|(?<![A-Za-z0-9_])\$?[A-Z]{1,3}\$?[0-9]{1,7}"
            r"(?::\$?[A-Za-z0-9_]{1,3}\$?[0-9]{1,7})?"
        )
        matched = list(re.finditer(pattern, formula))
        new = formula
        offset = 0
        for m in matched:
            raw = m.group(0)
    ...
    # Display named references and remapped formulas
    for name, (f, sheet, coord_set, minr, minc) in all_named_ref_info.items():
        ...
        st.expander(f"📌 Named Range: `{name}` → `{sheet}` in `{f}`").code("\n".join(entries))
